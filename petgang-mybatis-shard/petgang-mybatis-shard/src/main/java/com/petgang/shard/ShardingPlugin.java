...
/**
 * User: Xun Date: 13-6-3 Time: 下午2:49
 */
...
public class ShardingPlugin implements Interceptor {
    ...
    static {
        ...
        MAPPEDSTATEMENT_FIELD = ReflectionUtils.findField(BaseStatementHandler.class,
                "mappedStatement");
        ReflectionUtils.makeAccessible(MAPPEDSTATEMENT_FIELD);
    }

    /**
     * MyBatis wraps the JDBC Connection with a logging proxy but Spring
     * registers the original connection so it should be unwrapped before
     * calling
     * {@code DataSourceUtils.isConnectionTransactional(Connection, DataSource)}
     * 
        ...
        if (Proxy.isProxyClass(realConn.getClass())) {
            ...
            if (handler instanceof ShardingMybatisConnectionInvocationHandler) {
                ...
                if (statementHandler instanceof RoutingStatementHandler) {
                    StatementHandler delegate = (StatementHandler) ReflectionUtils.getField(
                            DELEGATE_FIELD, statementHandler);
                    mappedStatement = (MappedStatement) ReflectionUtils.getField(
                            MAPPEDSTATEMENT_FIELD, delegate);
                } else {
                    mappedStatement = (MappedStatement) ReflectionUtils.getField(
                            MAPPEDSTATEMENT_FIELD, statementHandler);
                }
