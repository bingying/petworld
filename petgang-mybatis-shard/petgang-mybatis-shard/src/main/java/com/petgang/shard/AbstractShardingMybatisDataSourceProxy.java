...
/**
 * User: Xun Date: 13-6-3 Time: 下午3:22
 */
public abstract class AbstractShardingMybatisDataSourceProxy extends AbstractDataSource implements
        InitializingBean {
    ...
    private static final Log logger = LogFactory
            .getLog(AbstractShardingMybatisDataSourceProxy.class);
    ...
    /**
     * 根据mybatis的MappedStatement mappedStatement, Object
     * parameter路由到正确的DataSource
     * 
     ...
     * has been fetched yet (-> actual JDBC Connection default not known
     * yet).
     * <p>
     ...
     * Connection has been fetched yet (-> actual JDBC Connection default
     * not known yet).
     * <p>
     * This property accepts the int constant value (e.g. 8) as defined in
     * the {@link java.sql.Connection} interface; it is mainly intended for
     * programmatic use. Consider using the
     * "defaultTransactionIsolationName" property for setting the value by
     * name (e.g. "TRANSACTION_SERIALIZABLE").
     * <p>
     * If not specified, the default gets determined by checking a target
     * Connection on startup. If that check fails, the default will be
     * determined lazily on first access of a Connection.
    ...
    public void afterPropertiesSet() {
        ...
        if (this.defaultAutoCommit == null || this.defaultTransactionIsolation == null) {
            try {
                ...
                logger.warn(
                        "Could not retrieve default auto-commit and transaction isolation settings",
                        ex);
            }
        }
    }
    ...
    /**
     * Check the default connection properties (auto-commit, transaction
     * isolation), keeping them to be able to expose them correctly without
     * fetching an actual JDBC Connection from the target DataSource.
     * <p>
     * This will be invoked once on startup, but also for each retrieval of
     * a target Connection. If the check failed on startup (because the
     * database was down), we'll lazily retrieve those settings.
     * 
    ...
    /**
     * Return a Connection handle that lazily fetches an actual JDBC
     * Connection when asked for a Statement (or PreparedStatement or
     * CallableStatement).
     * <p>
     * The returned Connection handle implements the ConnectionProxy
     * interface, allowing to retrieve the underlying target Connection.
     * 
    ...
    public Connection getConnection() throws SQLException {
        return (Connection) Proxy.newProxyInstance(
                ...
                new ShardingMybatisConnectionInvocationHandler(this, this.defaultAutoCommit(), this
    ...
    public Connection getConnection(String username, String password) throws SQLException {
        return (Connection) Proxy.newProxyInstance(
                ConnectionProxy.class.getClassLoader(),
                new Class[] { ConnectionProxy.class },
                new ShardingMybatisConnectionInvocationHandler(this, username, password, this
                        .defaultAutoCommit(), this.defaultTransactionIsolation()));
    }
