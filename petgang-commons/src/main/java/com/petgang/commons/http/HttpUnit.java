...
public class HttpUnit {
	...
	private static RequestConfig REQUEST_CONFIG = RequestConfig.custom()
			.setSocketTimeout(10000).setConnectTimeout(2000)
			.setRedirectsEnabled(true).setConnectionRequestTimeout(10000)
			.build();

	public static Result get(String uri) throws ClientProtocolException,
			IOException {
	...
	public static Result get(URI uri) throws ClientProtocolException,
			IOException {
		HttpGet get = new HttpGet(uri);
		get.setConfig(REQUEST_CONFIG);
		CloseableHttpResponse response = client.execute(get);
	...
	public static Result post(String uri) throws ClientProtocolException,
			IOException {
	...
	public static Result post(String uri, Map<String, String> params)
		...
		if (params != null && params.size() > 0) {
			for (Entry<String, String> tmp : params.entrySet()) {
				parameters.add(new BasicNameValuePair(tmp.getKey(), tmp
						.getValue()));
			}
		}
	 ...
	 * @throws IOException
	 */
	public static Result post(String uri, File file, String name,
	 ...
	 * @throws IOException
	 */
	public static Result post(String uri, Map<String, File> fileMap,
			Map<String, String> params) throws ClientProtocolException,
			IOException {
		...
		if (params != null && !params.isEmpty()) {
			ContentType contenttype = ContentType.create("text/plain",
					Charsets.UTF_8);
			for (Entry<String, String> tmp : params.entrySet()) {
				entityBuilder.addTextBody(tmp.getKey(), tmp.getValue(),
						contenttype);
			}
		}
		HttpPost post = new HttpPost(uri);
	 ...
	 * @throws IOException
	 */
	public static Result post(String uri, byte[] bytes, String name,
			String filename, Map<String, String> params)
			throws ClientProtocolException, IOException {
		MultipartEntityBuilder entityBuilder = MultipartEntityBuilder.create();
		entityBuilder.setCharset(Charsets.UTF_8);
	...
	private static Result buildResult(CloseableHttpResponse response)
			throws IOException {
